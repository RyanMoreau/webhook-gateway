...
// LoadReadOnly reads a YAML config file without resolving secrets.
// Useful for tools (like the TUI) that only need route/destination metadata.
func LoadReadOnly(path string) (*Config, error) {
	...
	// Relax validation: allow insecure destinations and skip secret checks.
